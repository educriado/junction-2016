...
class Bot:
    ...
    def finish_conversation(self):
        ...
        for item in self.conversation.split():
            f_conv.write(item)
        f_conv.close()
    ...
    def answer_back(self):
        ...
        if random.randint(0, 100) % 2 == 0:
            max_topic = ''
            max_count = 0
            for topic, count in self.topics_count.iteritems():
                if count > max_count:
                    max_count = count
                    max_topic = topic
            ...
            key = random.choice(self.topics_count.keys())
            return key


    def threaded_call(self):
        thread = Thread(target=api_call, args=(self,))
        thread.start()
